...
export function generateNoteFrequencies(referenceFrequency = 440) {
  const noteFrequencies = noteNames.map((note, index) => {
    const semitoneStepsFromA4 = index - noteNames.indexOf("A4");
    const frequency =
      referenceFrequency * Math.pow(2, semitoneStepsFromA4 / 12);
